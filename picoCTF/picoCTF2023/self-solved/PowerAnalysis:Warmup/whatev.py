#!/usr/bin/env python3
import random, sys, time

with open("key.txt", "r") as f:
    SECRET_KEY = bytes.fromhex(f.read().strip())

jonathan = (
    ,
)

# Leaks one bit of information every operation
leak_buf = []
def extract_lsbs_from_xor_indices(data_byte, key_byte):
    index = data_byte ^ key_byte # create index 

    list_entry = jonathan[index] # look inside list at index

    lsb = list_entry & 0x01 # extract least significant bit from entry

    leak_buf.append(lsb) # append least significant bit to list buffer

# Simplified version of AES with only a single encryption stage
def do_stuff(plaintext, key):
    global leak_buf
    leak_buf = []

    for i in range(16):
        extract_lsbs_from_xor_indices

# Leak the number of 1 bits in the lowest bit of every SBox output
def encrypt_and_leak(plaintext):
    do_stuff(plaintext, SECRET_KEY)
    return leak_buf.count(1)

pt = input("Please provide 16 bytes of plaintext encoded as hex: ")
if len(pt) != 32:
    print("Invalid length")
    sys.exit(0)

pt = bytes.fromhex(pt)
print("leakage result:", encrypt_and_leak(pt))
